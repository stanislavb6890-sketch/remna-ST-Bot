...
| Переменная | Описание |
|------------|----------|
| `CLOAKNET_API_URL` | URL панели (обязательно) |
...
3. Если в панели задан кастомный конфиг — агент подставляет в инбаунд с тегом `CLOAKNET-in` массив `users` из слотов. Иначе собирается минимальный конфиг по протоколу ноды.
...
```bash
docker build -f singbox-node/Dockerfile -t CLOAKNET/singbox-node:latest .
docker run -d \
  -e CLOAKNET_API_URL=https://your-panel.example.com \
  -e SINGBOX_NODE_TOKEN=your-token-from-admin \
  -e PROTOCOL=VLESS \
  -e PORT=443 \
  -p 443:443 \
  --restart unless-stopped \
  CLOAKNET/singbox-node:latest
```
